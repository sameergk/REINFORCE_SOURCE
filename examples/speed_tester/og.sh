...
if [ -z $print ]
then
        sudo ./build/speed_tester -l $cpu -n 3 --proc-type=secondary -- -r $service -n $inst_id -- -d 1
else
        sudo ./build/speed_tester -l $cpu -n 3 --proc-type=secondary -- -r $service -n $inst_id -- -d 1 -p $print
fi
