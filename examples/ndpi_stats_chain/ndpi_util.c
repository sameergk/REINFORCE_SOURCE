...
#include "ndpi_main.h"
#include "ndpi_util.h"
#include "ndpi_state.h"

#ifdef ENABLE_NFV_RESL
extern void *state_malloc(size_t size);
...
#ifdef ENABLE_NFV_RESL
  set_ndpi_malloc(state_malloc), set_ndpi_free(state_free);
...
#ifdef ENABLE_NFV_RESL
  detection_module_size = state_malloc(0) - (void*)module;
  printf("detection_module:%p current_mem:%p module_size:%lu(B)\n", module, state_malloc(0), detection_module_size);
#endif
#ifdef STATE_MEM_TEST
  module_mem_snapshot = malloc(detection_module_size);
  memcpy(module_mem_snapshot, (void*)module, detection_module_size);
  service_mem_snapshot = malloc(MAX_SERVICE_STATE_SIZE);
  memcpy(service_mem_snapshot, (void*)service_state, MAX_SERVICE_STATE_SIZE);
#endif

#ifdef ENABLE_NFV_RESL
  struct ndpi_workflow * workflow = service_state;
#else
...
#endif
  workflow->ndpi_flows_root = ndpi_malloc(workflow->prefs.num_roots * sizeof(void *));
  memset(workflow->ndpi_flows_root, 0, workflow->prefs.num_roots * sizeof(void *));
  return workflow;
}
...
#ifdef ENABLE_NFV_RESL
      //u_int32_t flow_idx = hashval % MAX_NF_STATE_ELEMENTS;
      u_int32_t flow_idx = (nf_flow_tbl_idx++) % MAX_NF_STATE_ELEMENTS;
      struct ndpi_flow_info *newflow = (struct ndpi_flow_info*)(nf_flow_tbl + flow_idx);
#else
...
#ifdef ENABLE_NFV_RESL
      newflow->flow_idx = flow_idx;
      //printf("new flow: tree_idx:%d, flow_idx:%d, %p %lu hash:%u\n", idx, flow_idx, newflow, MAX_NF_STATE_ELEMENTS, hashval);
#endif
...
#ifdef ENABLE_NFV_RESL
        newflow->ndpi_flow = &newflow->res_ndpi_flow; 
	memset(newflow->ndpi_flow, 0, SIZEOF_FLOW_STRUCT);
        newflow->src_id = &newflow->res_src_id; 
	memset(newflow->src_id, 0, SIZEOF_ID_STRUCT);
        newflow->dst_id = &newflow->res_dst_id; 
...
static struct ndpi_proto packet_processing(struct ndpi_workflow * workflow,
  ...
  if(flow != NULL) {
    ...
    flow->last_seen = time;
#ifdef ENABLE_NFV_RESL
        //printf("flow_idx:%d pkts: src2dst:%d dst2src:%d %p\n", flow->flow_idx, flow->src2dst_packets, flow->dst2src_packets, flow);
        update_dirty_nf_state_index(flow->flow_idx);
#endif
} else { // flow is NULL
...
/* ****************************************************** */
int state_memcmp(void *p1, void *p2, size_t n) {
        unsigned char u1, u2;
        int i, diff=0;
        int last=0, onek_chunk=0;
        //printf("state_memcmp:%zu\n", n);
        for(i=0; i<n; i++, p1++,p2++) {
                u1 = *(unsigned char *)p1;
                u2 = *(unsigned char *)p2;
                if (u1 != u2) { 
...
int state_memcmp_chunk(void *p1, void *p2, size_t n, size_t chunk_size) {
        unsigned char u1, u2;
        int i, diff=0;
        int c, last=0, chunk_diff=0;
        //printf("state_memcmp_chunk:%zu\n", n);
        for(i=0; i<n; i++, p1++,p2++) {
                u1 = *(unsigned char *)p1;
                u2 = *(unsigned char *)p2;
                if (u1 != u2) {
        ...
        printf("chunk_diff:%d, byte_diff:%d\n", chunk_diff, diff);
        return chunk_diff;
}
...
struct ndpi_proto ndpi_workflow_process_packet (struct ndpi_workflow * workflow,
 ...
 iph_check:
  ...
  /* process the packet */
  struct ndpi_proto rc_proto=packet_processing(workflow, time, vlan_id, iph, iph6,
			   ip_offset, header->caplen - ip_offset, header->caplen);
  
#ifdef ENABLE_NFV_RESL
  update_dirty_service_state_index(0);
#endif
#ifdef STATE_MEM_TEST
  //int rc1=state_memcmp(module_mem_snapshot, (void*)workflow->ndpi_struct, detection_module_size);
  printf("module_memcmp: size:%lu\n", detection_module_size);
  int rc2=state_memcmp_chunk(module_mem_snapshot, (void*)workflow->ndpi_struct, detection_module_size, 1000);
  memcpy(module_mem_snapshot, (void*)workflow->ndpi_struct, detection_module_size);

  printf("service_memcmp size:%lu\n", MAX_SERVICE_STATE_SIZE);
  //rc1=state_memcmp(service_mem_snapshot, (void*)workflow, MAX_SERVICE_STATE_SIZE);
  int rc3=state_memcmp_chunk(service_mem_snapshot, (void*)workflow, MAX_SERVICE_STATE_SIZE, 1000);
  memcpy(service_mem_snapshot, (void*)service_state, MAX_SERVICE_STATE_SIZE);
#endif
