...
function usage {
        echo "$0 CPU-LIST SERVICE-ID CLIENT_IFACE SERVER_IFACE SERVER_CONFIG  INST_ID [-p PRINT] [-n NF-ID]"
...
exec sudo $SCRIPTPATH/build/load_balancer -l $cpu -n 3 --proc-type=secondary -- -r $service -n $inst_id -- -c $client -s $server -f $config $print
